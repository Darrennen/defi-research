...
interface ApiResponse { reserves: Reserve[]; chain: string; chainIcon: string; chains: Record<string, ChainMeta>; fetchedAt: string; error?: string }
...
const CHAIN_SCAN: Record<string, string> = {
  'ethereum-core': 'https://etherscan.io/address', arbitrum: 'https://arbiscan.io/address',
  base: 'https://basescan.org/address', polygon: 'https://polygonscan.com/address',
  optimism: 'https://optimistic.etherscan.io/address', avalanche: 'https://snowtrace.io/address',
}

export default function AavePage() {
  const [chainKey, setChainKey] = useState('ethereum-core')
  ...
  const load = useCallback(async (chain: string) => {
    ...
    try {
      ...
      setReserves(data.reserves); setChains(data.chains); setChainName(data.chain)
      setChainIcon(data.chainIcon); setFetchedAt(data.fetchedAt); setSelected(0); setCfgOpen(false)
  ...
  return (
    <div>
      ...
      <div className="page-header" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            ...
            <div className="kicker" style={{ margin: 0 }}>Aave V3 · Reserve Risk Exposures</div>
          </div>
          <h1>On-Chain <em>Lending Risk</em></h1>
          <p className="dek">Per-asset reserve-level risk view across Aave V3 on multiple chains.</p>
        </div>
      ...
      <div className="ch-row">
        ...
        {!Object.keys(chains).length && [...Array(6)].map((_, i) => <div key={i} className="skel" style={{ height: 32, width: 100 }} />)}
      </div>
      ...
      {loading && !reserves.length && (
        <div style={{ marginTop: 24 }}>
          <div className="ch-row">{[...Array(8)].map((_, i) => <div key={i} className="skel" style={{ height: 32, width: 110, flexShrink: 0 }} />)}</div>
      ...
      {!loading && reserves.length > 0 && (
        <>
          ...
          <div className="kpi kpi-3col" style={{ marginTop: 24 }}>
            {[
              { l: 'Total Supplied', v: u(totalSupplied), d: chainName },
          ...
          {r && (
            <>
              <p style={{ fontFamily: 'var(--sans)', fontSize: 11, color: 'var(--ink-mute)', margin: '16px 0 16px' }}>
                <strong style={{ color: 'var(--ink)' }}>{r.symbol}</strong> on Aave V3 · {chainName}
              </p>
                  ...
                  <div className="cfg-body">
                    ...
                    <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--rule-soft)', display: 'flex', gap: 16 }}>
                      {[['Asset', r.assetAddress], ['aToken', r.aTokenAddress], ['vDebt', r.vDebtAddress]].map(([lbl, addr]) => (
                        <a key={lbl} href={`${scanBase}/${addr}`} target="_blank" rel="noopener noreferrer" style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--blue-ink)', textDecoration: 'underline', textUnderlineOffset: 3 }}>{lbl} ↗</a>
                      ))}
                    </div>
                  </div>
                )}
              ...
              <p style={{ marginTop: 24, fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--ink-mute)', letterSpacing: '0.04em' }}>
                Data from {chainName} RPC + DeFiLlama Yields API · {fetchedAt ? `Fetched ${new Date(fetchedAt).toLocaleTimeString()}` : 'Live'} · LT = Liquidation Threshold
              </p>
            </>
          )}
        </>
